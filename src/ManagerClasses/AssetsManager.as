	...
	public class AssetsManager
	{
			...
			public static function getTexture(name:String):Texture
			{
				...
				if (_gametextures[name] == undefined || _gametextures[name] == null)
				{
					...
					_gametextures[name] = Texture.fromBitmap(bitmap,false); //mipmaps false
				}
			...
			//-------------------------------o
			public static function loadTextureFromFile(path:String, name:String, callback:Function, showLoadingScreen:Boolean = true):void
			{
				loadQueCount++;
				
				if(showLoadingScreen)
				Core.getInstance().controlBus.appUIController.showLoadingScreen("LOADING", false, HexColours.NAVY_BLUE);
				...
				function onComplete ( e : Event ):void
				{
					...
					if (loadQueCount <= 0) {
					//remove the loading screen in the onLoaded() callback in each screen state, this allows any GPU freezes from initiating items to be covered, not just the loading process
					loadQueCount = 0;
					callback();
					}
