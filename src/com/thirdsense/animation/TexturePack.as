package com.thirdsense.animation
{
	...
	public class TexturePack 
	{
		...
		/*public function getWarpImage( asRenderTexture:Boolean=false, frame:int=0, helper_sequence:String = "" ):WarpImage
		{
			var img:WarpImage;
			
			if ( !this.atlas ) {
				if ( asRenderTexture ) {
					img = new WarpImage( this.render_texture );
				} else {
					img = new WarpImage( this.texture );
				}
			} else {
				var textures:Vector.<Texture> = this.atlas.getTextures( helper_sequence );
				img = new WarpImage( textures[frame] );
			}
