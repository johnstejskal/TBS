	...
	public class SpriteSheetHelper 
	{
		...
		static public function greedyArrange( rects:Vector.<Rectangle> ):Array
		{
			...
			while ( !success )
			{
				...
				for ( var i:uint = 0; i < len; i++ )
				{
					rects[i].x = Math.floor(rects[i].x);
					rects[i].y = Math.floor(rects[i].y);
					rects[i].width = Math.ceil(rects[i].width);
					rects[i].height = Math.ceil(rects[i].height);
					...
					{
						rectangles[i].x = 0;
						rectangles[i].y = rectangles[i - 1].y + lh;
						lh = rectangles[i].height + 1;
					}
