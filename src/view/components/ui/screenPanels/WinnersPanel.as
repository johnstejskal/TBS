package view.components.ui.screenPanels 
{
	...
	import staticData.dataObjects.CompetitionData;
	...
	import staticData.valueObjects.WeeklyWinnerVO;
	 ...
	 */
	public class WinnersPanel extends SuperPanel
	{
		private const DYNAMIC_TA_REF:String = "WinnersPanel";
		...
		private function init(e:Event):void 
		{
			...
			//-------o
			//content Area
			mc = LaunchPad.getAsset(PublicSettings.DYNAMIC_LIBRARY_UI, "TA_contentWinners") as MovieClip;
			for (var i:int = 0; i < CompetitionData.arrWeeklyWinners.length; i++) 
			{
				trace("CompetitionData.arrWeeklyWinners:" + CompetitionData.arrWeeklyWinners);
				mc["$tx"+(i+1)].text = "WEEK " + (i+1) + ": " + WeeklyWinnerVO(CompetitionData.arrWeeklyWinners[i]).label;
			}
			
			TexturePack.createFromMovieClip(mc, DYNAMIC_TA_REF, "TA_contentWinners", null, 1, 1, null, 0)
			_simContent = TexturePack.getTexturePack(DYNAMIC_TA_REF, "TA_contentWinners").getImage();
